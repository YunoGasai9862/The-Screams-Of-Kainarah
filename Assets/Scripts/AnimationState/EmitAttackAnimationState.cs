...
public class EmitAttackAnimationState : StateMachineScene
{
    ...
    {
        StateEvent = await (await (await GetBaseScene()).GetSceneUtilsAsync()).GetCustomEvent<StateEvent>();
    }

    // OnStateEnter is called when a transition starts and the state machine starts to evaluate this state
    override public void OnStateEnter(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        StateEvent.Invoke(new GenericStateBundle<EmitAnimationStateBundle<bool>, AttackState>()
        {
